...
var (
	...
	metricsListenerTarget = regexp.MustCompile("^[a-zA-Z0-9_]*$")
)
...
type MarathonSDConfig struct {
	...
	RefreshInterval model.Duration `yaml:"refresh_interval,omitempty"`
	TLSConfig       TLSConfig      `yaml:"tls_config,omitempty"`
	MetricsListener string         `yaml:"metrics_listener,omitempty"`
...
func (c *MarathonSDConfig) UnmarshalYAML(unmarshal func(interface{}) error) error {
	*c = DefaultMarathonSDConfig
	...
	reMatch := metricsListenerTarget.MatchString(c.MetricsListener)
	if len(c.MetricsListener) > 0 && !reMatch {
		c.MetricsListener = ""
	}
